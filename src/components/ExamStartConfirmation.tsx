...
const ExamStartConfirmation = ({ data }: { data: tExam | null }) => {
  ...
  const isTeacher = localStorage.getItem("role");
    ...
    <div>
      <div className="bg-gray-200 rounded-md shadow-lg p-5 w-[80vw]">
        <h1 className="flex border-b border-gray-100 py-1 justify-between items-center">
          <span className="font-normal">পরিক্ষার তারিখ:</span>
          <span className="font-medium text-sm">{dateFormate(data?.createdAt as string)}</span>
        </h1>
        <h1 className="flex border-b border-gray-100 py-1 justify-between items-center">
        ...
        <h1 className="flex border-b border-gray-100 py-1 justify-between items-center">
          ...
          <span className="font-medium text-sm">
            {data?.questionPapper?.subject}
          </span>
        </h1>
        <h1 className="flex border-b border-gray-100 py-1 justify-between items-center">
          <span className="font-normal">অধ্যায়:</span>
          <span className="font-medium text-sm">
            {data?.questionPapper?.name}
          </span>
        </h1>
        <h1 className="flex border-b border-gray-100 py-1 justify-between items-center">
          <span className="font-normal">মোট প্রশ্ন:</span>
          <span className="font-medium text-sm">
            {data?.questionPapper?.questions?.length} টি
          </span>
        </h1>
        <h1 className="flex border-b border-gray-100 py-1 justify-between items-center">
        ...
        <h1 className="text-red-500 mt-4 font-medium">
          সতর্কতা :- "Start Exam" বাটনে চাপ দিলে পরীক্ষা শুরু হয়ে যাবে, সকল
          প্রকার প্রস্তুতি নিয়ে পরিক্ষা শুরু করবে।
        </h1>
      </div>
